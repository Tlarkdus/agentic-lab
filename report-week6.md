...
- **산출물 경로**: `_incident/incident-A.log`(18줄)·`incident-B.log`(19줄),
  `.claude/skills/triage/SKILL.md`(①~⑥), `_brain/postmortem/payment-pool-exhaustion.md`,
  `_brain/decision/triage-gate.md`
- **핵심 증거 (11-1 대조군)**: `2ec1fd1 docs: fake incidents`. A는 시각·배포·증상이 연결되고
  (03:10 배포 → 03:11 pool 82% → 03:13 100% → 03:15 5xx 34%), B는 흩어져 있다
  (image-resize 지연 12건이 8시간에 불규칙, 서로 다른 3개 서비스의 `ECONNRESET` 3건 전부 retry 성공,
  48시간 무배포, 에러율 baseline)
- **핵심 증거 (11-2 triage)**: `8f5dbcd feat: triage skill`. A 리포트 — 원인 1순위가 v2.3.1 커넥션 누수,
  근거는 풀 사용률의 **단조 증가**(82% → 100%, 내려온 지점 없음). 조치는 전부 제안에 머물렀다
  (롤백·재시작·pool 상향을 표로 제시하되 "무엇을 확인한 뒤"를 붙임). **Confidence: High**
- **핵심 증거 (11-3 게이트 비교)**: `37dd052 feat: escalation gate`.
  A = High/Large → **알림 + on-call**(결제 핵심 경로, 실패 1,183건, 5xx 34→41% 악화 중) /
  B = Low/Small → **on-call**(사용자 영향 0건, 에러율 baseline 복귀). 같은 스킬, 같은 표, 다른 판정
- **핵심 증거 (11-4 순환)**: `3d220db docs: postmortem ingest`. 노드 2개 신설 + `index.md` 2행·`log.md` 1행.
  `[[링크]]` 6개 전수 검사 — 전부 실제 노드로 해결. wiki-query "payment 장애 때 뭘 배웠지?" →
  교훈 3건을 노드 인용으로 답하고, 노드가 스스로 적어둔 한계 2건("누수는 로그만으로 확정되지 않았다",
  "`npm test`는 이 사고를 못 잡는다")과 "교재용 가짜 장애"라는 경고까지 함께 반환
- **관찰 1 (절제가 설계였다)**: 원인이 High confidence로 좁혀졌는데도 스킬은 롤백을 실행하지 않았다.
  실행하지 않는 게 무능이 아니라 명세였다 — 새벽 3시에 확신에 찬 에이전트가 롤백을 눌러버리는 쪽이
  훨씬 무섭다. 진단과 조치 사이에 사람을 남겨두는 한 줄이 이 스킬에서 가장 중요한 줄이었다
- **관찰 2 (영향 0건인데 사람을 불렀다)**: B의 판정이 흥미로웠다. 사용자 영향 0, 에러율 정상, 악화 추세 없음 —
  모든 숫자가 "괜찮다"는데 판정은 on-call 호출이었다. **확신이 없다는 건 "별일 아니다"라고 말할 근거도
  없다는 뜻**이기 때문이다. 조용한 로그가 조용한 실패와 구분되지 않는다는 이번 주 주제의 마지막 사례
- **관찰 3 (정상 지표가 원인을 좁혔다)**: A에서 후보를 배제한 줄은 에러가 아니라 정상 로그였다 —
  `db-primary p99 18ms`(`:11`)와 `refund-service pool 3/50`(`:16`). 에러만 grep했으면 두 줄 다 놓쳤고
  원인은 세 갈래로 남았을 것이다. 사고 로그에서 "아무 일 없음"이 가장 비싼 정보였다
- **관찰 4 (자산이 자산을 먹였다)**: 이 LAB에서 새로 배운 기술은 없었다. LAB 03의 `_brain` 스키마,
  LAB 09의 근거 인용 강제, LAB 02의 등급 판정이 그대로 재조립됐을 뿐이다.
  `[[triage]]`로 링크를 걸었다가 노드가 아니라 스킬이라 깨진 링크가 된 것도, LAB 03의 규칙이
  아직 살아 있어서 걸린 것이다
